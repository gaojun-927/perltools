...
* author: 
*/
/* generated by software of Gao Jun ver=$version*/
/* start of lib */
TEXTHEAD
my $libhead='';
my $cellhead="cell_footprint : $subckt; ";
my $clkname='!CLKNAME!';
my $pinhead="capacitance : 0.01 ;";
my $anahead='';
my $timinginhead=<<'TIMEIN';
			timing() {
				  related_pin : "$clkname";
...
my @digitaltiminginpinlist;
my @digitaltimingoutpinlist;
my @digitaltiminginoutpinlist;
my @clkpinlist;
...
$timinginhead=~s/"\$clkname"/"$clkname"/g;		
...
HERE
	if( $subckt{$pinlist[$i]}->{'type'} eq '1' ){ #analog
		...
		print FILE $clkhead;
	}elsif( $subckt{$pinlist[$i]}->{'type'} eq '3'  and $subckt{$pinlist[$i]}->{'direction'} eq 'B') {
		print FILE $timinginhead;
		print FILE $timingouthead;
		print FILE "\n/*???????????????????*/\n";
		print " Warning, $subckt{$pinlist[$i]}->{'type'} is input and output with timing\n";
	}else{
		print " Warning, for $pinlist[$i],  $subckt{$pinlist[$i]}->{'type'} is unknow\n";
	}	
	print FILE "		}\n";
}	
...
for(my $i=0;$i<@pinlist;$i++){
	if( !exists($subckt{$pinlist[$i]}->{'min'}) ){
		print FILE <<PIN;
$count,	$pinlist[$i] ,	$typemap{$subckt{$pinlist[$i]}->{'type'}}/$dirmap1{$subckt{$pinlist[$i]}->{'direction'}} ,	Note:
PIN
	$count=$count+1;
	}else{
		my $temp=$count+$subckt{$pinlist[$i]}->{'max'}-$subckt{$pinlist[$i]}->{'min'};
		print FILE <<PIN;
$count - $temp,	$pinlist[$i] [$subckt{$pinlist[$i]}->{'max'}:$subckt{$pinlist[$i]}->{'min'}] ,	$typemap{$subckt{$pinlist[$i]}->{'type'}}/$dirmap1{$subckt{$pinlist[$i]}->{'direction'}} ,	Note:
PIN
		$count=$temp+1;
	}
}
